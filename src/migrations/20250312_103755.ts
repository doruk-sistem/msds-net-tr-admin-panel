...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   ALTER TABLE "msds_docs" DISABLE ROW LEVEL SECURITY;
  ALTER TABLE "msds_contents_msds_content" DISABLE ROW LEVEL SECURITY;
  ALTER TABLE "msds_contents" DISABLE ROW LEVEL SECURITY;
  DROP TABLE "msds_docs" CASCADE;
  DROP TABLE "msds_contents_msds_content" CASCADE;
  DROP TABLE "msds_contents" CASCADE;
  
  ALTER TABLE "payload_locked_documents_rels" DROP CONSTRAINT IF EXISTS "payload_locked_documents_rels_msds_docs_fk";
  ALTER TABLE "payload_locked_documents_rels" DROP CONSTRAINT IF EXISTS "payload_locked_documents_rels_msds_contents_fk";
  
  DROP INDEX IF EXISTS "payload_locked_documents_rels_msds_docs_id_idx";
  DROP INDEX IF EXISTS "payload_locked_documents_rels_msds_contents_id_idx";
  ...
  ALTER TABLE "msds_v2" ADD COLUMN "expiry_date" timestamp(3) with time zone;
  ALTER TABLE "payload_locked_documents_rels" DROP COLUMN IF EXISTS "msds_docs_id";
  ALTER TABLE "payload_locked_documents_rels" DROP COLUMN IF EXISTS "msds_contents_id";
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  CREATE TABLE IF NOT EXISTS "msds_docs" (
  	...
  	"preparation_date" varchar,
  	"form_no" varchar,
  	"new_regulation_date" varchar,
  	"how_many_regulations" varchar,
  ...
  ALTER TABLE "payload_locked_documents_rels" ADD COLUMN "msds_docs_id" integer;
  ALTER TABLE "payload_locked_documents_rels" ADD COLUMN "msds_contents_id" integer;
  DO $$ BEGIN
  ...
  ALTER TABLE "msds_v2" DROP COLUMN IF EXISTS "expiry_date";`)
}
