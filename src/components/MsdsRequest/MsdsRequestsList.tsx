...
type MsdsRequest = {
  ...
  createdAt: string
  url?: string // sdsFile yerine direkt url
  filename?: string // ve filename ekleyelim
}
export const MsdsRequestsList = () => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/msdsRequestsV2?where[requestedBy][equals]=${user.id}&depth=2`, {
        credentials: 'include',
      })
            ...
            filteredData.map((request) => (
              <TableRow key={request.id}>
                ...
                <TableCell className="text-right">
                  {request.url ? ( // sdsFile yerine direkt url kontrolü yapalım
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={async () => {
                        try {
                          const fileUrl = request.url; // Direkt url'i kullanalım
                          const filename = request.filename; // Direkt filename'i kullanalım
