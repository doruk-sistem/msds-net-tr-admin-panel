...
import { Column, useConfig, usePreferences, useServerFunctions } from '@payloadcms/ui'
import { SortColumnProps } from '@payloadcms/ui/elements/SortColumn'
import { ColumnPreferences } from '@payloadcms/ui/providers/ListQuery'
...
export const TableColumnsProvider: React.FC<Props> = ({
  ...
  const prevCollection = React.useRef<SanitizedCollectionConfig['slug']>(collectionSlug as any)
  const { getPreference } = usePreferences()

  const [tableColumns, setTableColumns] = React.useState(columnState)
  const tableStateControllerRef = React.useRef<any>(null)
  ...
  const moveColumn = useCallback(
    async (args: { fromIndex: number; toIndex: number }) => {
      ...
      if (result) {
        setTableColumns(result.state as any)
  ...
  const setActiveColumns = React.useCallback(
    async (activeColumnAccessors: string[]) => {
      ...
      setTableColumns(columnState as any)
      setTable(Table)
    },
    [
      tableColumns,
  ...
  const resetColumnsState = React.useCallback(async () => {
    await setActiveColumns(defaultColumns as any)
  ...
  React.useEffect(() => {
    const sync = async () => {
      ...
      if (collectionHasChanged || !listPreferences) {
        ...
        prevCollection.current = collectionSlug as any
